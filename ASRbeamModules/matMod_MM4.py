...
def matCalc_concrete(eps0, deps, time, dtime, predef, dpredef, matProps, stress, stiff, stateVars):
    ...
    ft = (1-eps_asr/(eps_asr+0.001))*ft
    
    #--STRAIN CALCULATIONS--

    dpredef = max(dpredef,0.)
    ...
    sigcr,dsigcr_dalpha = sigcr_calc(E_0, ft, 0.001, beta_E, eps_asr, alpha, alpha_critical)
    fcr = sig - sigcr
...
def sigcr_calc(E_0, ft, gamma, beta_E, eps_asr, alpha, alpha_critical):
    ...
    if alpha ==0.:#<= alpha_critical:
        ...
        dsigcr_dalpha = -ft#*np.exp(-gamma*alpha/ft*E_0)*gamma/ft*E_0
    else:
        sigcr = 0.001*ft
        dsigcr_dalpha = 0.0
